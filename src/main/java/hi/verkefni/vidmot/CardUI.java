...
public class CardUI extends AnchorPane {
     ...
     */
    public void setCardUI(Card s){
        for (int i = 0; i < 4; i++){
            for (int j = 0; j < 5; j++){
                if (j > 0 && j < 3){
                    ((Label)getChildren().get(j)).setText(s.getValue() + "");
                } else if (j > 2){
                    ((ImageView)getChildren().get(j)).setImage(readImages(s));
                }
            }
        }
    }
     ...
     */
    private Image readImages(Card s){
        String sort = "myndir/" + s.getSuit().name().toLowerCase() + ".png";
        return new Image(CardUI.class.getResourceAsStream(sort));
    }
}
